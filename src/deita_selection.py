...
if __name__ == '__main__':
    ...
    parser.add_argument('--full_dataset_root', default='../sample_data', type=str)
    parser.add_argument('--deita_data', default='../sample_data/deita_data', type=str)
    parser.add_argument('--deita_embedding_path', default='/scratch', type=str)
    args = parser.parse_args()

    if not os.exists(args.deita_data):
        os.mkdir(args.deita_data)

    files = os.listdir(args.full_dataset_root)
    for file in files: 
        try:
            print(f'{file} is being processed ...')            
            
            dataset = file.split('_')[1].replace('.jsonl','')
            print(f'Processing {dataset} ... Starting timer ...')
            start_time = time.time()

            embedding_model_path = os.path.join(args.deita_embedding_path, 'embeddings', f'{dataset}.pkl')
        
            embed_pipeline = Pipeline("embed_pipeline", 
                                    data_path = os.path.join(args.full_dataset_root, file),   # json file with sharegpt format
            ...
            embed_pipeline.run()
            print(f'Embedding Construction Successful ... Time taken for embedding construction: {(time.time() - start_time)/60} minutes.')
            del embed_pipeline
            gc.collect()
            start_time = time.time()

            pipeline = Pipeline("score_pipeline", 
                    data_path = os.path.join(args.full_dataset_root, file),   
            ...
            pipeline.run()
            print(f'Scoring Successful for {dataset} ... Time taken for scoring: {(time.time() - start_time)/60} minutes. Moving on to filtering ...')
            del pipeline
            gc.collect()
            ...
            filter_pipeline = Pipeline("filter_pipeline", 
                        data_path = os.path.join(args.deita_data, 'scores', f'{dataset}.json'),  # json file with sharegpt format
                        ...
                        output_path = os.path.join(args.deita_data, f'{dataset}_filtered_data.json'),  # json format output path
                        device=0)
            filter_pipeline.run()
            print(f'Filtering Successful for {dataset} ... Time taken for filtering: {(time.time() - start_time)/60} minutes.')
            del filter_pipeline
            gc.collect()
        except: 
            gc.collect()
