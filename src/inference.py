...
from utils import model_formatter, dump_iffeval_predictions
import torch
import os
import tqdm
import json
...
def inference(test_inputs, model, sampling_params):
    with tqdm.tqdm(torch.no_grad()):
        ...
        predictions = [
            intermediate_output.outputs[0].text
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="<path to your llama checkpoint>",
    )
    parser.add_argument(
        "--model_type",
        type=str,
        help="Required to format the prompts in accordance with the model class.",
    )
    ...
    parser.add_argument(
        "--benchmarks",
        nargs="+",
        choices=["lima", "alpacaeval", "ifeval", "infobench"],
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="if openLLM evaluations are to be conducted, needs to be passed",
    )
    parser.add_argument(
        "--tasks",
        nargs="+",
    ...
    if not args.eval_harness:
        ...
        for benchmark in args.benchmarks:
            if benchmark == "infobench":
                ...
                inputs = [
                    record["input"] + " " + record["instruction"] for record in dataset
                ]
            ...
            dump_predictions(
                inputs,
                outputs,
                os.path.join(args.output_path, benchmark, model_identifier),
                args.model_path,
            )
