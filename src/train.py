    ...
    def generate(self, prompt, max_new_tokens):
        ...
        with torch.inference_mode():
            ...
            predictions = self.tokenizer.decode(
                output[0][len(prompt[0]) :], skip_special_tokens=True
            )
        return predictions
...
def main():
    ...
    args.max_train_samples = min(
        args.budget, len(train_dataset)
    )  # Setting max samples to training data budget
    ...
    def format_instruction(model_input):
        ...
        if args.model_type == "llama2":
            return [
                f"""<s> [INST] <<SYS>> {SYSTEM_INSTRUCTION} <</SYS>>{model_input} [/INST]"""
            ]
    ...
    def preprocess_train_function(examples):
        ...
        try:
            inputs = [
                ex["input"] for ex in examples["task"]
            ]  # For FLAN-type datasets where the task-split was necessary to assert uniform sampling per task.
    ...
    if training_args.do_train:
        ...
        if (
            "random" in training_args.output_dir
        ):  # To maintain strict randomness in the sampling.
            generated_seed = random.choice(range(args.max_train_samples))
            train_dataset = train_dataset.shuffle(seed=generated_seed)
            print(
                f"Have reshuffled dataset for random sampling with {generated_seed}..."
            )
            train_dataset = train_dataset.select(range(args.max_train_samples))
    ...
    if args.wandb_artefact_logging:
        callback = ComputeCallback(
            trainer, test_dataset, num_samples=args.max_eval_samples, max_new_tokens=256
        )  # Reduced max target length since we are only sanity checking
