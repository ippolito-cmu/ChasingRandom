...
def model_formatter(prompts, model_identifier, system_instruction=SYSTEM_INSTRUCTION):
    if "gemma" in model_identifier:
        ...
        return [
            f"""<s> [INST] <<SYS>> {SYSTEM_INSTRUCTION} <</SYS>>{prompt} [/INST]"""
            for prompt in prompts
        ]
    else:
        print(
            "WARNING: Model identifier did not have parent identifier. Defaulting to LLaMa"
        )
        return [
            f"{system_instruction} \n ### Instruction: {prompt} \n ### Response:"
            for prompt in prompts
...
def eval_harness_summary_compiler(
    inference_root="../results", compiled_result_root="../compiled_results"
):
...
def batch_process(batch_size, model, tokenizer, prompts, max_new_tokens):
    ...
    while batch_ctr < len(prompts):
        ...
        local_batch = tokenizer(
            local_batch_prompt,
            return_tensors="pt",
            truncation=True,
            padding="max_length",
            max_length=512,
        )  # Has to be enough to include the prompt as well as the response.
        with torch.no_grad():
            intermediate_outputs = model.generate(
                input_ids=local_batch.input_ids.cuda(),
                attention_mask=local_batch.attention_mask.cuda(),
                max_new_tokens=max_new_tokens,
                pad_token_id=tokenizer.eos_token_id,
            )
            predictions = tokenizer.batch_decode(
                intermediate_outputs[:, local_batch.input_ids.shape[-1] :],
